...
* CS261 - Assignment 4
* Random test implementation of card smithy
*/
...
int testCard(int testNum) {
    ...
    int card = smithy;
	int i, j, r, compare, fail, test_fail,
		choice1, choice2, choice3, handPos, bonus, curPlayer;    
    i = j = r = compare = fail = test_fail =
	...
	int numPlayers = rand() % (MAX_PLAYERS+1 - 2) + 2;	// random 2-4 players
   	int kingdomCards[] = {adventurer, council_room, feast, gardens, mine, remodel, 
	...
	if( (testG.handCount[curPlayer] != (G.handCount[curPlayer] + 2)) 
		|| (testG.deckCount[curPlayer] != (G.deckCount[curPlayer] - 3)) )
		test_fail++;

	// Draw cards from own deck 	
...
int main() {
	...
	i = test = test_fail = 0;    
	printf ("*** RANDOM TESTING card smithy: ***\n");
	sleep(2);
